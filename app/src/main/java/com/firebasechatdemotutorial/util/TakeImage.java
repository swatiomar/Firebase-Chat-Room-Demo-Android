package com.firebasechatdemotutorial.util;
...
//				imageFileUri = FileProvider.getUriForFile(TakeImage.this,BuildConfig.APPLICATION_ID + ".provider",new File(path));
				imageFileUri = FileProvider.getUriForFile(TakeImage.this,"com.eventapp.provider",new File(path));
...
//	compressing image
	private String compressImage(Bitmap finalBitmap, int width, int height)
	{
		...
		{
			bitmap=Bitmap.createScaledBitmap(finalBitmap, width/2, height/2, true);
		}
		else
		{
			bitmap=finalBitmap;
